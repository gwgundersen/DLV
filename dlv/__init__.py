...
from dlv.basics.loadData import loadData
from dlv.safety_check.regionSynth import regionSynth
from dlv.safety_check.precisionSynth import precisionSynth
from dlv.safety_check.safety_analysis import safety_analysis
...
from dlv.mcts.mcts_twoPlayer import mcts_twoPlayer
...
def solve():
    ...
    succNum = 0
    for whichIndex in range(cfg.startIndexOfImage,cfg.startIndexOfImage + cfg.dataProcessingBatchNum):
        print "\n\nprocessing input of index %s in the dataset: " %(str(whichIndex))
        succ = handleOne(model,dc,whichIndex)
        if succ == True: succNum += 1
    dc.addSuccPercent(succNum/float(cfg.dataProcessingBatchNum))
    dc.provideDetails()
    dc.summarise()
    dc.close()
...
def handleOne(model,dc,startIndexOfImage):
    ...
    image = cfg.NN.getImage(model,startIndexOfImage)
    ...
    while k <= cfg.maxLayer:
        ...
        layerType = networkBasics.getLayerType(model, k)
        re = False
        ...
        if layerType in ["Convolution2D","Conv2D", "Dense", "InputLayer"] and k >= 0 :
            ...
            (originalClass,originalConfident) = cfg.NN.predictWithImage(model,image)
            origClassStr = cfg.dataBasics.LABELS(int(originalClass))

            path0="%s/%s_original_as_%s_with_confidence_%s.png"%(cfg.directory_pic_string,startIndexOfImage,origClassStr,originalConfident)
            cfg.dataBasics.save(-1,originalImage, path0)
            ...
            while f < cfg.numOfFeatures :
                ...
                while True and index != (-1,-1):
                    ...
                    cfg.dataBasics.save(index[0],image0,path2)
                    ...
                    if t == k:
                        ...
                        (nextSpan,nextNumSpan,rs,wk,rk) = safety_analysis(model,cfg.dataset,t,startIndexOfImage,st,index,nextSpan,nextNumSpan,npre)
                        if len(rk) > 0:
                            rk = (zip (*rk))[0]
                ...
                if re == True:
                    ...
                    (ocl,ocf) = cfg.NN.predictWithImage(model,wk[0])
            ...
            while st.terminalNode(st.rootIndex) == False and st.terminatedByControlledSearch(st.rootIndex) == False and runningTime_all <= cfg.MCTS_all_maximal_time:
                ...
                diffs = st.diffImage(st.rootIndex)
                path0="%s/%s_temp_%s.png"%(cfg.directory_pic_string,startIndexOfImage,len(diffs))
                cfg.dataBasics.save(-1,image1,path0)
                (newClass,newConfident) = cfg.NN.predictWithImage(model,image1)
                ...
                path0="%s/%s_currentBest.png"%(cfg.directory_pic_string,startIndexOfImage)
                cfg.dataBasics.save(-1,image1,path0)
            ...
            if re == True:
                path0="%s/%s_%s_%s_modified_into_%s_with_confidence_%s.png"%(cfg.directory_pic_string,startIndexOfImage,"sift_twoPlayer", origClassStr,newClassStr,newConfident)
                cfg.dataBasics.save(-1,image1,path0)
                path0="%s/%s_diff.png"%(cfg.directory_pic_string,startIndexOfImage)
                cfg.dataBasics.save(-1,np.subtract(image,image1),path0)
            ...
            newClassStr = cfg.dataBasics.LABELS(int(newClass))
            re = newClass != originalClass
            path0="%s/%s_%s_modified_into_%s_with_confidence_%s.png"%(cfg.directory_pic_string,startIndexOfImage,origClassStr,newClassStr,newConfident)
            cfg.dataBasics.save(-1,image1,path0)
